...
class DataTrainingArguments:
    ...
    test_file: Optional[str] = field(default=None, metadata={"help": "A data dir of test files."})
    task_name: Optional[str] = field(default='deft_task3', metadata={"help": "A data dir of test files."})
    qa_type: bool = field(
...
def main():
    ...
    datasets = load_dataset('text', data_files={
        ...
        "validation": data_args.dev_file,
        "test": data_args.test_file
    })

    # Labels
    # Some models have set the order of the labels to use, so let's make sure we do use it.
    label_list = ["Direct-Defines", "Refers-To", "Indirect-Defines", "Supplements", "AKA",
                  "fragment", ]
    ...
    def preprocess_function(examples):
        ...
        if data_args.qa_type:
            ...
            sentence_1 = ['Whats the relation between the two entities noted with [ER] and [EH]?' for _ in
                          range(len(examples['text']))]
            sentence_2 = [line.strip().split("\t")[0] for line in examples['text']]
